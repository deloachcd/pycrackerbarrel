...
def position_in_triangle(i, j):
    return (j <= i and i < 5 and j < 5 and i >= 0 and j >= 0)
...
class CrackerBarrelTriangle:
    ...
    def __init__(self, **kwargs):
        ...
        if 'remove_peg' in kwargs.keys():
            ...
            if not position_in_triangle(*r):
                msg = 'Invalid hole specified for initial peg removal'
                raise Exception(msg)
            else:
                self.remove_peg(*r)
    ...
    def peg_present(self, i, j):
        if position_in_triangle(i, j):
            ...
            msg = 'Checked for peg presence in location not in triangle'
            raise Exception(msg)
    ...
    def remove_peg(self, i, j):
        if position_in_triangle(i, j):
            ...
            msg = 'Requested peg removal for location not in triangle'
            raise Exception(msg)
    ...
    def place_peg(self, i, j):
        if position_in_triangle(i, j):
            ...
            msg = 'Requested peg placement for location not in triangle'
            raise Exception(msg)
